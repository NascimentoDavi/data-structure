...
public class Abb {
    ...
    public LCItem camCentral(){
        LCItem vet = new LCItem(this.quant);
        return CamCentral(vet, this.root);
    }

    private LCItem camCentral(LCItem vet, NoAbb node) {
        if(node != null){
            // walk
            vet = camCentral(vet, no.getLeft());
            vet.insereFinal(no.getItem());  
            vet = camCentral(vet, no.getRight());
        }
        return vet;
    }
}